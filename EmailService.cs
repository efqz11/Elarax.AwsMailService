...
namespace Elarax.AwsMailService
{
    public class EmailService : IEmailService
    {
        ...
        public MailMessage[] SendMultipleEmails(params MailMessage[] mailMessages)
        {
            var succeeded = new List<MailMessage>();
            var actions = new Action[mailMessages.Length];
            var tasks = new Task[mailMessages.Length];
            for (var i = 0; i < mailMessages.Length; i++)
            {
                var mailMessage = mailMessages[i];
                Action action = () =>
                {
